...
def setupPokemon():
    ...
    bulbasaur       = Pokemon("Bulbasaur",          grassType,      45,     49,     49)
    ivysaur         = Pokemon("Ivysaur",            grassType,      60,     62,     63)
    venusaur        = Pokemon("Venusaur",           grassType,      80,     82,     83)
    charmander      = Pokemon("Charmander",         fireType,       39,     52,     43)
    charmeleon      = Pokemon("Charmeleon",         fireType,       58,     64,     58)
    charizard       = Pokemon("Charizard",          fireType,       78,     84,     78)
    squirtle        = Pokemon("Squirtle",           waterType,      44,     48,     65)
    wartortle       = Pokemon("Wartortle",          waterType,      59,     63,     80)
    blastoise       = Pokemon("Blastoise",          waterType,      79,     83,     100)
    caterpie        = Pokemon("Caterpie",           bugType,        45,     30,     35)
    metapod         = Pokemon("Metapod",            bugType,        50,     20,     55)
    butterfree      = Pokemon("Butterfree",         bugType,        60,     45,     50)
    weedle          = Pokemon("Weedle",             bugType,        40,     35,     30)
    kakuna          = Pokemon("Kakuna",             bugType,        65,     25,     50)
    beedrill        = Pokemon("Beedrill",           bugType,        65,     80,     40)
    pidgey          = Pokemon("Pidgey",             normalType,     40,     45,     40)
    pidgeotto       = Pokemon("Pidgeotto",          normalType,     63,     60,     55)
    pidgeot         = Pokemon("Pidgeot",            normalType,     83,     80,     75)
    rattata         = Pokemon("Rattata",            normalType,     30,     56,     35)
    raticate        = Pokemon("Raticate",           normalType,     55,     81,     60)
    spearow         = Pokemon("Spearow",            normalType,     40,     60,     30)
    fearow          = Pokemon("Fearow",             normalType,     65,     90,     65)
    ekans           = Pokemon("Ekans",              poisonType,     35,     60,     44)
    arbok           = Pokemon("Arbok",              poisonType,     60,     85,     69)
    pikachu         = Pokemon("Pikachu",            electricType,   35,     55,     30)
    raichu          = Pokemon("Raichu",             electricType,   60,     90,     55)
    sandshrew       = Pokemon("Sandshrew",          groundType,     50,     75,     85)
    sandslash       = Pokemon("Sandslash",          groundType,     75,     100,    110)
    nidoranFemale   = Pokemon("Nidoran Female",     poisonType,     55,     47,     52)
    nidorina        = Pokemon("Nidorina",           poisonType,     70,     62,     67)
    nidoqueen       = Pokemon("Nidoqueen",          poisonType,     90,     82,     87)
    nidoranMale     = Pokemon("Nidoran Male",       poisonType,     46,     57,     40)
    nidorino        = Pokemon("Nidorino",           poisonType,     61,     72,     57)
    nidoking        = Pokemon("Nidoking",           poisonType,     81,     92,     77)
    clefairy        = Pokemon("Clefairy",           normalType,     70,     45,     48)
    clefable        = Pokemon("Clefable",           normalType,     95,     70,     73)
    vulpix          = Pokemon("Vulpix",             fireType,       38,     41,     40)
    ninetales       = Pokemon("Ninetales",          fireType,       73,     76,     75)
    jigglypuff      = Pokemon("Jigglypuff",         normalType,     115,    45,     20)
    wigglytuff      = Pokemon("Wigglytuff",         normalType,     140,    70,     45)
    zubat           = Pokemon("Zubat",              poisonType,     40,     45,     35)
    golbat          = Pokemon("Golbat",             poisonType,     75,     80,     70)
    oddish          = Pokemon("Oddish",             grassType,      45,     50,     55)
    gloom           = Pokemon("Gloom",              grassType,      60,     65,     70)
    vileplume       = Pokemon("Vileplume",          grassType,      75,     80,     85)
    paras           = Pokemon("Paras",              bugType,        35,     70,     55)
    parasect        = Pokemon("Parasect",           bugType,        60,     55,     50)
    venonat         = Pokemon("Venonat",            bugType,        70,     65,     60)
    venomoth        = Pokemon("Venomoth",           bugType,        10,     55,     25)
    diglett         = Pokemon("Diglett",            groundType,     35,     80,     50)
    dugtrio         = Pokemon("Dugtrio",            groundType,     40,     45,     35)
    meowth          = Pokemon("Meowth",             normalType,     65,     70,     60)
    persian         = Pokemon("Persian",            normalType,     50,     52,     48)
    psyduck         = Pokemon("Psyduck",            waterType,      80,     82,     78)
    golduck         = Pokemon("Golduck",            waterType,      40,     80,     35)
    mankey          = Pokemon("Mankey",             fightingType,   65,     105,    60)
    primeape        = Pokemon("Primape",            fightingType,   55,     70,     45)
    growlithe       = Pokemon("Growlithe",          fireType,       90,     110,    80)
    arcanine        = Pokemon("Arcanine",           fireType,       40,     50,     40)
    poliwag         = Pokemon("Poliwag",            waterType,      65,     65,     65)
    poliwhirl       = Pokemon("Poliwhirl",          waterType,      90,     85,     95)
    poliwrath       = Pokemon("Poliwrath",          waterType,      25,     20,     15)
    abra            = Pokemon("Abra",               psychicType,    40,     35,     30)
    kadabra         = Pokemon("Kadabra",            psychicType,    55,     50,     45)
    alakazam        = Pokemon("Alakazam",           psychicType,    70,     80,     50)
    machop          = Pokemon("Machop",             fightingType,   80,     100,    70)
    machoke         = Pokemon("Machoke",            fightingType,   90,     130,    80)
    machamp         = Pokemon("Machamp",            fightingType,   50,     75,     35)
    bellsprout      = Pokemon("Bellsprout",         grassType,      65,     90,     50)
    weepinbell      = Pokemon("Weepinbell",         grassType,      80,     105,    65)
    victreebel      = Pokemon("Victreebel",         grassType,      40,     40,     35)
    tentacool       = Pokemon("Tentacool",          waterType,      80,     105,    65)
    tentacruel      = Pokemon("Tentacruel",         waterType,      40,     80,     100)
    geodude         = Pokemon("Geodude",            rockType,       55,     95,     115)
    graveler        = Pokemon("Graveler",           rockType,       80,     110,    130)
    golem           = Pokemon("Golem",              rockType,       50,     85,     55)
    ponyta          = Pokemon("Ponyta",             fireType,       65,     100,    70)
    rapidash        = Pokemon("Rapidash",           fireType,       90,     65,     65)
    slowpoke        = Pokemon("Slowpoke",           waterType,      95,     49,     49)
    slowbro         = Pokemon("Slowbro",            waterType,      25,     35,     70)
    magnemite       = Pokemon("Magnemite",          electricType,   50,     60,     95)
    magneton        = Pokemon("Magneton",           electricType,   52,     65,     55)
    farfetchd       = Pokemon("Farfetch'd",         normalType,     35,     85,     45)
    doduo           = Pokemon("Duduo",              normalType,     60,     110,    70)
    dodrio          = Pokemon("Dudrio",             normalType,     65,     45,     55)
    seel            = Pokemon("Seel",               waterType,      90,     70,     80)
    dewgong         = Pokemon("Dewgong",            waterType,      80,     80,     50)
    grimer          = Pokemon("Grimer",             poisonType,     105,    105,    75)
    muk             = Pokemon("Muk",                poisonType,     30,     65,     100)
    shellder        = Pokemon("Shellder",           waterType,      50,     95,     180)
    cloyster        = Pokemon("Cloyster",           waterType,      30,     35,     30)
    gastly          = Pokemon("Gastly",             ghostType,      30,     35,     30)
    haunter         = Pokemon("Haunter",            ghostType,      45,     50,     45)
    gengar          = Pokemon("Gengar",             ghostType,      60,     65,     60)
    onix            = Pokemon("Onix",               rockType,       35,     45,     160)
    drowzee         = Pokemon("Drowzee",            psychicType,    60,     48,     45)
    hypno           = Pokemon("Hypno",              psychicType,    85,     73,     70)
    krabby          = Pokemon("Krabby",             waterType,      30,     105,    90)
    kingler         = Pokemon("Kingler",            waterType,      55,     130,    115)
    voltorb         = Pokemon("Voltorb",            electricType,   40,     30,     50)
    electrode       = Pokemon("Electrode",          electricType,   60,     50,     70)
    exeggcute       = Pokemon("Exeggcute",          grassType,      60,     40,     80)
    exeggutor       = Pokemon("Exeggutor",          grassType,      95,     95,     85)
    cubone          = Pokemon("Cubone",             groundType,     50,     50,     95)
    marowak         = Pokemon("Marowak",            groundType,     60,     80,     110)
    hitmonlee       = Pokemon("Hitmonlee",          fightingType,   50,     120,    53)
    hitmonchan      = Pokemon("Hitmonchan",         fightingType,   50,     105,    79)
    lickitung       = Pokemon("Lickitung",          normalType,     90,     55,     75)
    koffing         = Pokemon("Koffing",            poisonType,     40,     65,     95)
    weezing         = Pokemon("Weezing",            poisonType,     65,     90,     120)
    rhyhorn         = Pokemon("Rhyhorn",            groundType,     80,     85,     95)
    rhydon          = Pokemon("Rhydon",             groundType,     105,    130,    120)
    chansey         = Pokemon("Chansey",            normalType,     250,    5,      5)
    tangela         = Pokemon("Tangela",            grassType,      65,     55,     115)
    kangaskhan      = Pokemon("Kangaskhan",         groundType,     105,    95,     80)
    horsea          = Pokemon("Horsea",             waterType,      30,     40,     70)
    seadra          = Pokemon("Seadra",             waterType,      55,     65,     95)
    goldeen         = Pokemon("Goldeen",            waterType,      45,     67,     60)
    seaking         = Pokemon("Seaking",            waterType,      80,     92,     65)
    staryu          = Pokemon("Staryu",             waterType,      30,     45,     55)
    starmie         = Pokemon("Starmie",            waterType,      60,     75,     85)
    mrmime          = Pokemon("Mr Mime",            psychicType,    40,     45,     65)
    scyther         = Pokemon("Scyther",            bugType,        70,     110,    80)
    jynx            = Pokemon("Jynx",               iceType,        65,     50,     35)
    electabuzz      = Pokemon("Electabuzz",         electricType,   65,     50,     35)
    magmar          = Pokemon("Magmar",             fireType,       65,     83,     57)
    pinsir          = Pokemon("Pinsir",             bugType,        65,     95,     57)
    tauros          = Pokemon("Tauros",             normalType,     75,     100,    95)
    magikarp        = Pokemon("Magikarp",           waterType,      20,     10,     55)
    gyrados         = Pokemon("Gyrados",            waterType,      95,     125,    79)
    lapras          = Pokemon("Lapras",             waterType,      130,    85,     80)
    ditto           = Pokemon("Ditto",              normalType,     48,     48,     48)
    eevee           = Pokemon("Eevee",              normalType,     55,     55,     50)
    vaporean        = Pokemon("Vaporean",           waterType,      130,    65,     60)
    jolteon         = Pokemon("Jolteon",            electricType,   65,     65,     65)
    flareon         = Pokemon("Flareon",            fireType,       65,     130,    65)
    porygon         = Pokemon("Porygon",            normalType,     65,     60,     70)
    omanyte         = Pokemon("Omanyte",            rockType,       35,     40,     100)
    omastar         = Pokemon("Omastar",            rockType,       70,     60,     125)
    kabuto          = Pokemon("Kabuto",             rockType,       30,     80,     90)
    kabutops        = Pokemon("Kabutops",           rockType,       60,     115,    105)
    aerodactyl      = Pokemon("Aerodactyl",         rockType,       80,     105,    65)
    snorlax         = Pokemon("Snorlax",            normalType,     160,    110,    65)
    articuno        = Pokemon("Articuno",           iceType,        90,     85,     100)
    zapdos          = Pokemon("Zapdos",             electricType,   90,     90,     85)
    moltres         = Pokemon("Moltres",            fireType,       90,     100,    90)
    dratini         = Pokemon("Dratini",            dragonType,     41,     64,     45)
    dragonair       = Pokemon("Dragonair",          dragonType,     61,     84,     65)
    dragonite       = Pokemon("Dragonite",          dragonType,     91,     134,    95)
    mewtwo          = Pokemon("Mewtwo",             psychicType,    106,    110,    90)
    mew             = Pokemon("Mew",                psychicType,    100,    100,    100)
